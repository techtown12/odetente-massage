...
const ECOMMERCE_STORE_ID = "store_01KNGT6XQJM0RJY96MBT2NC0MA";
...
const extractVariants = (variants) => {
  return (variants || []).map((v) => {
    ...
    const originalCurrency = v?.prices?.[0]?.currency_code || "eur";
    // Forcer CAD pour ce store (spa Montréal)
    const currency = "cad";
    const originalCurrencyInfo = v?.prices?.[0]?.currency;

    // Créer une config CAD forcée
    const currencyInfo = {
    ...
    return {
      ...
      price_formatted: formatCurrency(price_in_cents, currencyInfo),
